  ...
  return (
    <section className="px-6 py-6 pb-32">
       <div className="flex items-center justify-between mb-6 px-2">
         <h3 className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.2em]">Charity Impact</h3>
       ...
       <div className="grid grid-cols-2 gap-4">
         {highLights.map((item, i) => (
           <motion.div 
             key={i}
             whileHover={{ y: -5 }}
             className="p-5 bg-gradient-to-br from-zinc-900 to-black border border-white/5 rounded-3xl"
           >
             <div className={`w-10 h-10 ${item.color} rounded-xl mb-4 flex items-center justify-center text-white shadow-lg shadow-black/40`}>
               {item.icon}
             </div>
             <h4 className="text-[11px] font-black uppercase tracking-tight text-white mb-2">{item.name}</h4>
             <p className="text-[9px] text-zinc-500 font-bold leading-tight uppercase tracking-widest">{item.desc}</p>
           </motion.div>
         ))}
       </div>
    </section>
